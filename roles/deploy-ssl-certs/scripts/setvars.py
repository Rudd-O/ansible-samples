...
safeify = lambda x: x.replace(os.path.sep, "_").replace(os.path.pardir, "_" * len(os.path.pardir))
...
for k, v in i.items():
    ...
    o[k]["assembled_certificate"]["path"] = incertdir("assembled_" + safeify(k) + ".crt")
    o[k]["intermediate_certificates"] = []
    for n, int in enumerate(v['intermediates']):
        o[k]["intermediate_certificates"].append({})
        o[k]["intermediate_certificates"][-1]["path"] = incertdir(("intermediate_%s_" % n) + safeify(k) + ".crt")
